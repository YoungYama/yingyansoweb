...
KindEditor.plugin('map', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		var iframeHtml = ['<!doctype html><html><head>',
			...
			'			});',
			'		} else {',
			'			myAlert("Invalid address: " + address);',
			'		}',
			'	});',
			'}',
			'</script>',
		...
		function reloadPage(path, order, func) {
			...
			K.ajax(K.addParam(fileManagerJson, param + '&' + new Date().getTime()), function(data) {
				...
				if(data['error'] != undefined && data['error'] == 1) {
					var zIndex = document.getElementsByClassName("ke-dialog-mask")[0].style.zIndex;
					document.getElementsByClassName("ke-dialog-mask")[0].style.zIndex = zIndex - 3;

					var newDialogs = [];
					var filemanagerDialog = null;
					for(var i in self.dialogs) {
						if(self.dialogs[i].name == "filemanager") {
							filemanagerDialog = self.dialogs[i];
						} else {
							newDialogs.push(self.dialogs[i]);
						}
					}
					...
					yesBtn: {
						...
						click: function(e) {
							...
							if(url == 'http://' || K.invalidUrl(url)) {
								myAlert(self.lang('invalidUrl'));
								urlBox[0].focus();
								return;
							}
							if(!/^\d*$/.test(width)) {
								myAlert(self.lang('invalidWidth'));
								widthBox[0].focus();
								return;
							}
							if(!/^\d*$/.test(height)) {
								myAlert(self.lang('invalidHeight'));
								heightBox[0].focus();
								return;
							}
				...
				var uploadbutton = K.uploadbutton({
					...
					afterUpload: function(data) {
						...
						if(data.error === 0) {
							...
							myAlert(self.lang('uploadSuccess'));
						} else {
							myAlert(data.message);
						}
					},
					...
					click: function(e) {
						...
						if(showLocal && showRemote && tabs && tabs.selectedIndex === 1 || !showRemote) {
							if(uploadbutton.fileBox.val() == '') {
								myAlert(self.lang('pleaseSelectFile'));
								return;
							}
							dialog.showLoading(self.lang('uploadLoading'));
							uploadbutton.submit();
...
KindEditor.plugin('multiimage', function(K) {
	...
	self.plugin.multiImageDialog = function(options) {
		...
		if(uploadJson.indexOf("/file/uploads") == -1) {
			uploadJson = uploadJson.replace("/file/upload", "/file/uploads");
		}
	...
	********************************** */
	...
	SWFUpload.Console.writeLine = function(message) {
		...
		try {
			...
			myAlert("Exception: " + ex.name + " Message: " + ex.message);
		}
	};
})();
(function() {
			...
			var dialog = self.createDialog({
					...
					yesBtn: {
						...
						click: function(e) {
							...
							if(!/^\d*$/.test(padding)) {
								myAlert(self.lang('invalidPadding'));
								paddingBox[0].focus();
								return;
							}
							if(!/^\d*$/.test(spacing)) {
								myAlert(self.lang('invalidSpacing'));
								spacingBox[0].focus();
								return;
							}
							if(!/^\d*$/.test(border)) {
								myAlert(self.lang('invalidBorder'));
								borderBox[0].focus();
								return;
							}
							if(table) {
