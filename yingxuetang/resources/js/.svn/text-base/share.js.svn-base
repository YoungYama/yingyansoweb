...
(function($, window, document, undefined) {
	...
	function init(target, options) {
		...
		var $msb_main = "<a class='msb_main'><img title='分享' src='" + getRootPath() +  "/yingxuetang/resources/images/share_core_square.jpg'></a>";
		var $social_group = "<div class='social_group'>" +
			"<a class='msb_network_button weixin' title='微信'>weixin</a>" +
			//"<a class='msb_network_button tQQ'>tQQ</a>" +
			"<a class='msb_network_button sina' title='微博'>sina</a>" +
			"<a class='msb_network_button qZone' title='QQ空间'>qZone</a>" +
			//"<a class='msb_network_button douban' title='豆瓣'>douban</a>" +
			"</div>";
		...
		//添加腾讯微博分享事件
		$(document).on("click", ".social_group .tQQ", function() {
			tQQ(this, settings);
			addShareCount();
		});
		//添加QQ空间分享事件
		$(document).on("click", ".social_group .qZone", function() {
			qZone(this, settings);
			addShareCount();
		});
		//添加新浪微博分享事件
		$(document).on("click", ".social_group .sina", function() {
			sinaWeibo(this, settings);
			addShareCount();
		});
		//添加豆瓣分享事件
		$(document).on("click", ".social_group .douban", function() {
			doubanShare(this, settings);
			addShareCount();
		});
		//添加微信分享事件
		$(document).on("click", ".social_group .weixin", function() {
...
//		var url = decodeURIComponent(options.url);
	...
	function showWX(url) {
		var weixing = '<div id="weixin">' +
			...
			'<span>分享到微信朋友圈</span>' +
			'<a href="javascript: void(0);" id="close" class="bd_weixin_popup_close">×</a>' +
			'</div>' +
			'<div class="erweima">' +
		...
		$("#weixin .erweimas").empty();
		$("#weixin .erweimas").qrcode({
